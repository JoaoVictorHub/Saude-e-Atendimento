...
function inicializarSistema() {
    checarAcesso();
    // Carrega os dados específicos do usuário logado (usando as chaves dinâmicas)
...
function salvarNovaSenha() {
    ...
    senhaResultado.classList.add('hidden');

    if (novaSenha.length < 8) {
        exibirFeedback('⚠️ A nova senha deve ter no mínimo 8 caracteres.', 'error', 'senhaResultado');
        return;
    }
    if (novaSenha !== confirmaNovaSenha) {
        exibirFeedback('⚠️ As senhas digitadas não coincidem.', 'error', 'senhaResultado');
        return;
    }
    if (alterarSenha(novaSenha)) {
        exibirFeedback('✅ Senha alterada com sucesso! Você será deslogado.', 'success', 'senhaResultado');
        // Desloga após um breve período para forçar o login com a nova senha
        setTimeout(() => {
            fecharModalAlterarSenha();
            deslogarUsuario(); 
        }, 1500);
    } else {
        exibirFeedback('❌ Erro ao alterar a senha. Tente novamente.', 'error', 'senhaResultado');
    }
}
